...
class CmdInterpreterFactory:
    ...
    def get_command_kind(self, command):
        ...
        regexps = []
        assert(ccregexp)
        regexps.append(("CC", re.compile("^(" + ccregexp + ")$")))
        assert(arregexp)
        regexps.append(("AR", re.compile("^(" + arregexp + ")$")))
        assert(ldregexp)
        regexps.append(("LD", re.compile("^(" + ldregexp + ")$")))
        for kind, regexp in regexps:
            m = regexp.match(basename)
            if m: return kind
        return "UNK"
    ...
    def test():
        ...
        tests = [
            ...
            {'command': {'args': ["ld", "-o", "main.exe", "a.o", "b.o"],
                         'cwd': "."},
             'expected_kind': "LD",
             'expected_inputs': ["a.o", "b.o"],
             'expected_outputs': ["main.exe"]
             },
            {'command': {'args': ["ld", "-shared", "-o", "liba.so",
                                  "a.o", "b.o"],
                         'cwd': "."},
             'expected_kind': "LD",
             'expected_inputs': ["a.o", "b.o"],
             'expected_outputs': ["liba.so"]
             },
            {'command': {'args': ["ld", "-r", "-o", "obj.o", "a.o", "b.o"],
                         'cwd': "."},
             'expected_kind': "LD",
             'expected_inputs': ["a.o", "b.o"],
             'expected_outputs': ["obj.o"]
             },
            {'command': {'args': ["armv7-linux-ld", "-r",
                                  "-o", "obj.o", "a.o", "b.o"],
                         'cwd': "."},
             'expected_kind': "LD",
             'expected_inputs': ["a.o", "b.o"],
    ...
    def test():
        ...
        cc = CCInterpreter(
            {'args': shlex.split("cc a.o b.o c.c d.ro e.os f.a"),
             'cwd': "."})
        assert(cc.ccld_interpreter() != None)
        ...
        cc = CCInterpreter(
            {'args': shlex.split("cc -o"),
             'cwd': "."})
        assert(cc.is_wellformed() == False)
        assert(cc.get_args() == ['cc', '-o'])
...
class LDInterpreter:
    """
    Returns information on a command line for a LD linker.
    The information is used by the atos-driver or the dependency builder.
    """
    class NameSpace():
        """ Simple container for argument values. """
    ...
    def parse_args_(self):
        ...
        arg0 = self.expanded_args_[0]
        args = self.expanded_args_[1:]
        parser = ld_argparse.LDArgumentParser(arg0, cwd)
        try:
            ns = parser.parse_args(args, self.NameSpace())
            interp = ld_argparse.LDArgumentInterpreter(ns)
        except (ld_argparse.LDArgumentError,
                gen_argparse.MalformedArgument):
            # If an error occurs in arguments parsing
            # we consider the command failing and ignore it.
            return None
    ...
    def ccld_interpreter(self):
        """
        Returns the LDArgumentInterpreter object,
        or None if command is invalid.
        """
        return self.interp_
    ...
    def get_kind(self):
        """
        Returns the command kind, actually a LD command.
        """
        return "LD"
    ...
    def get_input_pairs(self):
        """
        Get input files pairs (src_kind, fname) to be processed as
        dependencies from a LD command line args.
        It is invalid to call this function if interpreter() is None.
        """
        assert(self.interp_ != None)
        if self.input_pairs_ == None:
            cwd = self.command_['cwd']
            self.input_pairs_ = map(
                lambda x: ('SRC_LNK', os.path.normpath(os.path.join(cwd, x))),
