...
add_decl_attribute(const char *cur_func_name, acf_ftable_entry_t *acf_entry, tree decl, const char *acf_pass_name) {
 ...
 for (i = 0; i < acf_entry->attr_arg_number; i++) {
  ...
  break;
     case INT_TYPE:
  argument = build_int_cst ((tree) __null, acf_entry->opt_args[i].av.int_arg);
  argument_list = tree_cons_stat ((tree) __null,argument,argument_list );
  break;
     }
 }
...
add_global_attribute(const char *cur_func_name, acf_ftable_entry_t *acf_entry, const char *acf_pass_name) {
 ...
 if (acf_entry->opt_args[0].av.str_arg[0] == 'O') {
  ...
  if ((opt_str = strchr(acf_entry->opt_args[0].av.str_arg, '=')) != __null)
      opt_str ++;
  else
      opt_value = 1;
     }
...
# 715 "/opt/gcc-plugins/src/acf_plugin.c"
...
# 924 "/opt/gcc-plugins/src/acf_plugin.c"
