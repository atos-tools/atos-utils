...
graph()
{
    ...
    $ROOT/bin/atos lib create_db -C ${CFGNAME}-1
    $ROOT/bin/atos lib add_result -C ${CFGNAME}-1 \
	-r "target:X,variant:REF,time:100,size:100"

    $ROOT/bin/atos lib create_db -C ${CFGNAME}-2
    $ROOT/bin/atos lib add_result -C ${CFGNAME}-2 \
	-r "target:X,variant:REF,time:100,size:100"
