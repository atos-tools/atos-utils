...
graph()
{
    ...
    $ROOT/lib/atos/atos_lib.py create_db -C ${CFGNAME}-1
    $ROOT/lib/atos/atos_lib.py add_result -C ${CFGNAME}-1 \
	-r "target:X,variant:REF,time:100,size:100"

    $ROOT/lib/atos/atos_lib.py create_db -C ${CFGNAME}-2
    $ROOT/lib/atos/atos_lib.py add_result -C ${CFGNAME}-2 \
	-r "target:X,variant:REF,time:100,size:100"
