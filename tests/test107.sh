...
TEST_CASE="ATOS oprof.out format 6"
...
$ROOT/bin/atos-explore-acf --debug -f OPT-O2 2>&1 | tee LOG

[ `cat LOG | grep -i "error: oprofile output format not supported" | wc -l` -eq 0 ]

[ `cat LOG | grep "gen_file_by_file: hot objects" | wc -l` -eq 1 ]

[ `cat LOG | grep "gen_file_by_file: hot objects: \[\]" | wc -l` -eq 0 ]

[ `cat LOG | grep "gen_file_by_file: cold objects" | wc -l` -eq 1 ]

[ `cat LOG | grep "gen_file_by_file: cold objects: \[\]" | wc -l` -eq 0 ]
